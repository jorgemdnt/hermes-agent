...
export function startManualProviderOAuth(providerId: string, profile?: ProfileScope, opts?: { append?: boolean }) {
...
export async function startProviderOAuth(provider: OAuthProvider, ctx: OnboardingContext) {
  ...
  try {
    const start = await startOAuthLogin(provider.id, ctx.scope, { append: peekPendingOAuthAppend() })
